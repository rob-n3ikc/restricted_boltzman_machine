...
bool best_train_o_matic(int, float, int, float*);
...
float reconstruct_best( int, float *, float *); // find the best energy and use the hidden from that to select a new visible
float reconstruct( int, float *, float *); // use the best hidden to select a new visible; use all the hiddens

int in_bias;
bool clear_bias();
bool add_to_bias(int, float*);
bool normalize_bias();
};
