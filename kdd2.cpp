...
//		printf("%f %f\n", x,the_data[i]->data[j]);
	...
	for( int j=0; j< nfeature; j+=2)
	{
		float x;
		x = the_data[i]->data[j];
		if( x >=0. && x < 1./4){ the_data[i]->data[j] = -1.F; the_data[i]->data[j+1] = -1.F;  continue;}
		if( x >=1./4 && x < 2./4){ the_data[i]->data[j] = -1.F; the_data[i]->data[j+1] =  1.F;  continue;}
		if( x >=2./4 && x < 3./4){ the_data[i]->data[j] =  1.F; the_data[i]->data[j+1] = -1.F;  continue;}
		if( x >=3./4 ){ the_data[i]->data[j] =  1.F; the_data[i]->data[j+1] =  1.F;  continue;}
	}
