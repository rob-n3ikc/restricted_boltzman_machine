...
//	printf("\n");
	...
	if( strcmp("bias", tokens[0] ) == 0){
		if( inme < 3) {fprintf(stderr,"bias needs three arguments\n"); exit(0);}
		float b;
		int a;
		sscanf(tokens[1],"%d",&a);
		sscanf(tokens[2],"%f",&b);
		if( a > bias.size()){ fprintf(stderr,"bias %d is outside of the defined range of visible nodes\n",a); exit(0);}
		bias[a] = b;
		continue;
	}
...
float RBM::energy(int inme, float *data)
{
...
#ifndef BIAS
		visible[i]->fsign = data[i]; // this can be replaced by a callback.
#else
		visible[i]->fsign = data[i]-bias[i]; // this can be replaced by a callback.
#endif
	for( int i=0; i< hidden.size(); i++)
	{
		ep = 0.;
...
float RBM::best_raw_expert_energy(int inme, float *data)
{
// there is a design tradeoff here
// if every hidden node needs all its connections then there is
// a nhidden*nvisible memory requirement
...
float RBM::best_raw_expert_energy(int inme, int *data)
{
// there is a design tradeoff here
// if every hidden node needs all its connections then there is
// a nhidden*nvisible memory requirement
...
float RBM::reconstruct_best( int inme, float *data, float *recon)
{
	if( visible.size() != inme) return 0.;
	float e,ep,em;
	float enorm;
...
// energy is summed over every link for +/- 
		...
		hidden[i]->sign = -1;
		em += link->ef();
		enorm += fabs(link->weight*link->v->fsign);
	}//j
		if( enorm > 0.)
		{
			em /= enorm; ep /= enorm;
		}
                if( em < ebest){ ebest = em; ibest = i;}
                if( ep < ebest){ ebest = ep; ibest = i;}
		if( em < ep){ e+= em; hidden[i]->sign = -1;}
		else {e += ep; hidden[i]->sign = 1;}
	}//i
// now we know the best. and its sign is set right.
	for( int j=0; j< hidden[ibest]->links.size(); j++)
	{
		connect* link;
		for( int i=0; i< visible.size(); i++)
		{
		link = (connect*)hidden[ibest]->links[i]; // HIDDEN FULL RANK assumption.

		ep = link->weight *hidden[ibest]->sign;
		em = -ep;
		recon[i] = 1.;
		if( em < ep) recon[i] = -1.;
		}
	...
	return ccn*ccn/(ccd1*ccd2);
//	return sqrt(ccn*ccn/(ccd1*ccd2));
//	return true;
}

float RBM::reconstruct( int inme, float *data, float *recon)
{
	if( visible.size() != inme) return 0.;
	float e,ep,em;
	float enorm;
...
// energy is summed over every link for +/- 
	...
	for( int j=0; j< hidden[i]->links.size(); j++)
	{
		...
		{
		link = (connect*)hidden[i]->links[k]; // HIDDEN FULL RANK assumption.

		ep = link->weight *hidden[i]->sign;
		em = -ep;
		if( em < ep) recon[k] += -1.F;
		else recon[k] += 1.F;
		}//k
	}//j
	}//i
...
float RBM::best_expert_energy(int inme, float *data)
{
// there is a design tradeoff here
// if every hidden node needs all its connections then there is
// a nhidden*nvisible memory requirement
...
bool RBM::train(float beta, int inme, float *data)
{
	static int count = 0;
...
// Plus/Minus is arbitrary here
		ep = link->ef()*beta *hidden[i]->sign;
		em = -ep;
//		if( ep > 20.) ep = 20.;
//		if( ep < -20.) ep = -20.;
		dot = link->h->sign * links[j]->v->fsign;
		fp = exp( ep);
		fm = exp( em);  // safer
		damp =  (fp-fm)/(fp+fm);
	//	edot = (float)dot;
...
bool RBM::anti_train_o_matic(float delta, float beta, int inme, float *data)
{
	if( visible.size() != inme) return false;
	float e;
	e = energy( inme, data);
...
// find the best and worst energy links
	...
	for( int j=0; j< hidden[i]->links.size(); j++)
	{
		...
		link = (connect*)hidden[i]->links[j];
		my_energy +=  link->ef() ;
		my_norm += fabs(link->weight*link->v->fsign);
	}//j
...
//		if( ep < -20.) ep = -20.;
		...
		damp =  (fp-fm)/(fp+fm);
		damp *= dot*hidden[i]->sign;
//		link->weight -= (dot+damp)*DAMP;
// small uphill step.
		link->weight += delta*DAMP*(dot+damp);
//		link->weight -= edot;
	}//j
	}//i
	return true;
...
// contrast/divergence version
// basically IFF predicted right don't update
//   else update 2x.
bool RBM::best_train_o_matic(int niter_cd, float beta, int inme, float *data)
{
	if( visible.size() != inme) return false;
	float e;
	for( int ni = 0;ni < niter_cd; ni++)
	{
	e = energy( inme, data);
// at this stage all the signs are at the minimum energy.
	float eworst,ebest;
...
bool RBM::best_train_o_matic(float beta, int inme, float *data)
{
	if( visible.size() != inme) return false;
	float e;
	e = energy( inme, data);
...
bool RBM::train_o_matic(float beta, int inme, float *data)
{
	if( visible.size() != inme) return false;
	float e;
	e = energy( inme, data);
...
bool RBM::add_to_bias( int n, float *data)
{
	if( bias.size() != n) return false;
	in_bias += 1;
	for( int i=0; i < n; i++)
		bias[i] += data[i];
	return true;
...
bool RBM::normalize_bias()
{
	if( in_bias < 1) return false;
//	in_bias *= 10;
	for( int i=0; i< bias.size(); i++)
		bias[i] /= in_bias;
