...
bool RBM::reconstruct_best( int inme, float *data, float *recon)
{
	if( visible.size() != inme) return 0.;
	float e,ep,em;
	float enorm;
...
bool RBM::reconstruct( int inme, float *data, float *recon)
{
	if( visible.size() != inme) return 0.;
	float e,ep,em;
	float enorm;
...
//		damp *= dot;
		damp *= dot*hidden[i]->sign;
	//	link->weight -= dot+damp;
	//	link->weight += delta*(dot+damp);
		link->weight += delta*dot;
//		link->weight -= edot;
	}//j
	}//i
	return true;
